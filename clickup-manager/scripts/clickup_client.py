...
def main():
    ...
    if args.command == "configure":
        pass
    elif args.command == "list-spaces" and not args.team_id:
        spaces_parser.error("--team-id is required (or set CLICKUP_TEAM_ID)")
    ...
    elif (args.command == "list-tasks" or args.command == "create-task") and not args.list_id:
        # Note: we need to handle error for the specific subparser, but simpler to just print and exit or use general error
        print("Error: --list-id is required (or set CLICKUP_LIST_ID)")
        sys.exit(1)
