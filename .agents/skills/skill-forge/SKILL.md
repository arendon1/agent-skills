...
script: `init.py` — Scaffolds a new skill directory skeleton with Token Loading Hierarchy.
script: `audit.py` — Generates structured audit prompts for skill evaluation (structure, security, language, description).
script: `deploy.py` — Deploys slash-command workflows (`/skill-create`, `/skill-audit`, `/skill-improve`) to agent directories.
script: `utils.py` — Shared parsing utilities for reading SKILL.md frontmatter (name/description extraction).